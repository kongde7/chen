...
//#define LCD_ONE_CMD         0x80
//#define DISP_ENABLE_DISP        (0x04 | DISP_ENABLE)
//#define DISP_ENABLE_CUR         (0x02 | DISP_ENABLE)
//#define DISP_ENABLE_BLINK       (0x01 | DISP_ENABLE)
...
//void Cursor_OnOff(uint8_t OnOff);
...
void LCD_DisplayChar2 (uint16_t ln, uint16_t col, unsigned char c); //自己编写显示16*24字函数（字符）

void LCD_DisplayString  (unsigned int ln, unsigned int col, unsigned char *s);

void LCD_DisplayString2  (unsigned int ln, unsigned int col, unsigned char *s); //自己编写显示16*24字函数（字符串）
...
//以下为自己添加的部分清屏函数
void LCD_ClearScreen2 (unsigned short color);
